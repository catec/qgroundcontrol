...
GICA::GICA(QObject *parent)
    : QObject(parent), _connectionAvailable(false), _linearPosX(0.0f),
      _linearPosY(0.0f), _linearPosZ(0.0f), _linearVelX(0.0f),
      _linearVelY(0.0f), _linearVelZ(0.0f), _linearAccX(0.0f),
      _linearAccY(0.0f), _linearAccZ(0.0f), _angularPosRoll(0.0f),
      _angularPosPitch(0.0f), _angularPosYaw(0.0f), _angularVelRoll(0.0f),
      _angularVelPitch(0.0f), _angularVelYaw(0.0f), _angularAccRoll(0.0f),
      _angularAccPitch(0.0f), _angularAccYaw(0.0f), _miscIntegrity(0.0f),
      _miscNumSat(0), _miscSbasCorrection(false) {
...
void GICA::toggleGPS(bool enable) {
  Vehicle *vehicle =
...
void GICA::mavlinkMessageReceived([[maybe_unused]] LinkInterface *link,
  ...
  case MAVLINK_MSG_ID_GEOSUB_LINEAR_VALUES:
    ...
    if (_linearPosX != linear_values.x_pos)
      setLinearPosX(linear_values.x_pos);
    if (_linearPosY != linear_values.y_pos)
      setLinearPosY(linear_values.y_pos);
    if (_linearPosZ != linear_values.z_pos)
      setLinearPosZ(linear_values.z_pos);
    if (_linearVelX != linear_values.x_vel)
      setLinearVelX(linear_values.x_vel);
    if (_linearVelY != linear_values.y_vel)
      setLinearVelY(linear_values.y_vel);
    if (_linearVelZ != linear_values.z_vel)
      setLinearVelZ(linear_values.z_vel);
    if (_linearAccX != linear_values.x_acc)
      setLinearAccX(linear_values.x_acc);
    if (_linearAccY != linear_values.y_acc)
      setLinearAccY(linear_values.y_acc);
    if (_linearAccZ != linear_values.z_acc)
      setLinearAccZ(linear_values.z_acc);
  ...
  case MAVLINK_MSG_ID_GEOSUB_ANGULAR_VALUES:
    ...
    if (_angularPosRoll != angular_values.roll_pos)
      setAngularPosRoll(angular_values.roll_pos);
    if (_angularPosPitch != angular_values.pitch_pos)
      setAngularPosPitch(angular_values.pitch_pos);
    if (_angularPosYaw != angular_values.yaw_pos)
      setAngularPosYaw(angular_values.yaw_pos);
    if (_angularVelRoll != angular_values.roll_vel)
      setAngularVelRoll(angular_values.roll_vel);
    if (_angularVelPitch != angular_values.pitch_vel)
      setAngularVelPitch(angular_values.pitch_vel);
    if (_angularVelYaw != angular_values.yaw_vel)
      setAngularVelYaw(angular_values.yaw_vel);
    if (_angularAccRoll != angular_values.roll_acc)
      setAngularAccRoll(angular_values.roll_acc);
    if (_angularAccPitch != angular_values.pitch_acc)
      setAngularAccPitch(angular_values.pitch_acc);
    if (_angularAccYaw != angular_values.yaw_acc)
      setAngularAccYaw(angular_values.yaw_acc);
  ...
  case MAVLINK_MSG_ID_GEOSUB_MISC_VALUES:
    ...
    if (_miscIntegrity != misc_values.integrity)
      setMiscIntegrity(misc_values.integrity);
    if (_miscNumSat != misc_values.num_sat)
      setMiscNumSat(misc_values.num_sat);
    if (_miscSbasCorrection != misc_values.sbas_correction)
      setMiscSbasCorrection(misc_values.sbas_correction);
